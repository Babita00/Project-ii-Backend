  ...
  try {
    ...
    try {
      ...
      return next(
        new ApiError(
          500,
          `Error adding property to database: ${error.message}`,
        ),
      );
    }
  } catch (error) {
    console.error("Detailed error in addProperty:", error);
    return next(new ApiError(500, `Error adding property: ${error.message}`));
  }
});
